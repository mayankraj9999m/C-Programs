...
void mul_matrices(int r1, int c1, int r2, int c2, int matrix_1[r1][c1], int matrix_2[r2][c2], int new[r1][c2])
{
    for (int i = 0; i < r1; i++)
    {
        for (int j = 0; j < c2; j++)
        {
            ...
            for (int k = 0; k < r2; k++)
            {
                new[i][j] += matrix_1[i][k] * matrix_2[k][j];
            }
        }
    }
}
...
int main()
{
    ...
    {
        int result[r1][c2];
        mul_matrices(r1, c1, r2, c2, matrix_1, matrix_2, result);
